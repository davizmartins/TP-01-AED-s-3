...
public class ControleCurso {
    ...
    private void gerenciarInscritos(Curso curso) {
        try {
            ...
            while (true) {
                ...
                Usuario[] usuarios = new Usuario[inscricoes.size()];
                CursoUsuario[] inscricoesSalvas = inscricoes.toArray(new CursoUsuario[0]);

                for (int i = 0; i < inscricoes.size(); i++) {
                    usuarios[i] = controleInscricao.getArquivoInscricao().getArquivoUsuario()
    ...
    private void exportarListaInscritos(Usuario[] usuarios, String nomeCurso) {
        try {
            FileWriter writer = new FileWriter("inscritos.csv");
